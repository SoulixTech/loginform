...
const phoneEl = document.querySelector('.form-container .form-row input[name="phone"]');
const ageEl = document.querySelector('.form-container .form-row input[name="age"]');
const countryEl = document.querySelector('.form-container .form-row input[name="country"]');
const experienceEl = document.querySelector('.form-container .form-row select[name="experience"]');
const platformEl = document.querySelector('.form-container .form-row select[name="platform"]');
const messageEl = document.querySelector('.form-container .form-row textarea[name="message"]');
...
phoneEl.addEventListener('input', () => {
    phoneValid = phoneEl.value.length >= 10;
    if (phoneValid) {
...
ageEl.addEventListener('input', () => {
    ...
    ageValid = age >= 16 && age <= 100;
    if (ageValid) {
        ageEl.classList.add("valid");
    } else {
        ageEl.classList.remove("valid");
    }
})

// Country validation
countryEl.addEventListener('input', () => {
    countryValid = countryEl.value.length > 2;
    if (countryValid) {
        countryEl.classList.add("valid");
    } else {
        countryEl.classList.remove("valid");
    }
})

// Experience validation
experienceEl.addEventListener('change', () => {
    experienceValid = experienceEl.value !== '';
    if (experienceValid) {
        experienceEl.classList.add("valid");
    } else {
        experienceEl.classList.remove("valid");
    }
})

// Platform validation
platformEl.addEventListener('change', () => {
    platformValid = platformEl.value !== '';
    if (platformValid) {
        platformEl.classList.add("valid");
    } else {
        platformEl.classList.remove("valid");
    }
})

// Message validation
messageEl.addEventListener('input', () => {
    messageValid = messageEl.value.length > 20;
    if (messageValid) {
...
submitBtn.addEventListener('click', (e) => {
    const allValid = emailValid && checkboxEl.checked && nameValid && phoneValid && 
                     ageValid && countryValid && experienceValid && platformValid && 
                     messageValid && sprayRepeatCounter > 1;
    ...
    if (allValid) {
        ...
        gsap.to(".form-row", {
            ...
            stagger: .1,
            onComplete: () => {
                alert('🎉 Application Submitted Successfully!\n\nThank you for applying to our App Development Course.\nWe will contact you within 24 hours.');
                location.reload();
            }
        })
    } else {
        ...
        alert('⚠️ Please fill in all fields correctly:\n\n' +
              ...
              '✓ Wait for gears to spray the button!');
    }
})
        ...
        if (dIdx === (data.length - 1)) {
            tl.eventCallback("onUpdate", () => {
                ...
                if (deltaY > 8) {
                    ...
                    gsap.set(submitBtn, {
                        color: "rgba(255, 255, 255, " + state.sumbitBtnTextOpacity + ")"
                    })
                    gsap.set(sprayLines, {
                        attr: {
